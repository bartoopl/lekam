    ...
    <div class="contact-grid">
        ...
        <div class="contact-content">
            <p class="contact-text">
                {{ $contents['contact.intro.text1']->content ?? 'Masz pytania dotyczące naszych kursów? Skontaktuj się z nami, aby uzyskać szczegółowe informacje o szkoleniach, zapisach i punktach edukacyjnych.' }}
            </p>
            <p class="contact-text">
                {!! str_replace('Lek-am', '<span class="company-highlight">Lek‑am</span>', $contents['contact.intro.text2']->content ?? 'Możesz też porozmawiać ze swoim przedstawicielem firmy Lek-am, który udzieli Ci potrzebnych informacji i udzieli wsparcia w korzystaniu z serwisu.') !!}
            </p>
        </div>
        ...
        <div class="contact-info">
            ...
            <div class="info-section">
                <h3 class="info-title">{{ $contents['contact.lekam.title']->content ?? 'Informacja LEK-AM' }}</h3>
                <div class="contact-item">
                    <svg class="contact-icon" viewBox="0 0 24 24" fill="currentColor">
                        <path d="M6.62,10.79C8.06,13.62 10.38,15.94 13.21,17.38L15.41,15.18C15.69,14.9 16.08,14.82 16.43,14.93C17.55,15.3 18.75,15.5 20,15.5A1,1 0 0,1 21,16.5V20A1,1 0 0,1 20,21A17,17 0 0,1 3,4A1,1 0 0,1 4,3H7.5A1,1 0 0,1 8.5,4C8.5,5.25 8.7,6.45 9.07,7.57C9.18,7.92 9.1,8.31 8.82,8.59L6.62,10.79Z"/>
                    </svg>
                    {{ $contents['contact.lekam.phone']->content ?? '888 888 888' }}
                </div>
                <div class="contact-item">
                    <svg class="contact-icon" viewBox="0 0 24 24" fill="currentColor">
                        <path d="M20,8L12,13L4,8V6L12,11L20,6M20,4H4C2.89,4 2,4.89 2,6V18A2,2 0 0,0 4,20H20A2,2 0 0,0 22,18V6C22,4.89 21.1,4 20,4Z"/>
                    </svg>
                    {{ $contents['contact.lekam.email']->content ?? 'info@info.pl' }}
                </div>
            </div>

            <!-- Technical Support -->
            <div class="info-section">
                <h3 class="info-title">{{ $contents['contact.support.title']->content ?? 'Wsparcie techniczne serwisu' }}</h3>
                <div class="contact-item">
                    <svg class="contact-icon" viewBox="0 0 24 24" fill="currentColor">
                        <path d="M20,8L12,13L4,8V6L12,11L20,6M20,4H4C2.89,4 2,4.89 2,6V18A2,2 0 0,0 4,20H20A2,2 0 0,0 22,18V6C22,4.89 21.1,4 20,4Z"/>
                    </svg>
                    {{ $contents['contact.support.email']->content ?? 'office@creativetrust.pl' }}
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="footer-bottom">
        <div class="footer-bottom-left">
            <div style="display: flex; align-items: flex-end;">
                <span>&copy; {{ date('Y') }} Wszelkie Prawa zastrzeżone</span>
